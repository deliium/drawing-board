...
- Mutating WebSocket messages require a client `opId` (≤36); creates are idempotent on `(user_id, op_id)`.
...
- Frontend: unmatched inbound live stroke creates are non-authoritative; full reload uses `GET /api/strokes` and clears the in-memory WS queue.
