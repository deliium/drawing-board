...
- `DELETE /api/practice-data` clears **this user’s** practice attempts/progress (including review schedule) only — never free-board strokes or the account; default retention is keep-until-clear; self-serve account delete is unsupported — operator/DB cascade only.
...
- Passwords: bcrypt only; never log passwords, raw cookies, CSRF tokens, or full hashes.
