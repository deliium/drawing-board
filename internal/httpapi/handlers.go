...
func (a *API) ListStrokes(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[httpapi.ListStrokes] DEBUG userID=%d", uid)
	rows, err := a.Store.ListStrokesByUser(uid)
	if err != nil {
		log.Printf("[httpapi.ListStrokes] ERROR store userID=%d: %v", uid, err)
...
func (a *API) ClearStrokes(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[httpapi.ClearStrokes] DEBUG userID=%d", uid)
	before, listErr := a.Store.ListStrokesByUser(uid)
	if listErr != nil {
		log.Printf("[httpapi.ClearStrokes] ERROR list-before userID=%d: %v", uid, listErr)
	}
	if err := a.Store.ClearStrokesByUser(uid); err != nil {
		log.Printf("[httpapi.ClearStrokes] ERROR store userID=%d: %v", uid, err)
		writeJSON(w, 500, map[string]string{"error": err.Error()})
		return
	}
	count := 0
	...
	log.Printf("[httpapi.ClearStrokes] INFO clear completed userID=%d count=%d", uid, count)
...
func (a *API) DeleteStroke(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[httpapi.DeleteStroke] DEBUG userID=%d id=%d", uid, id)
	if err := a.Store.DeleteStroke(uid, id); err != nil {
		log.Printf("[httpapi.DeleteStroke] ERROR store userID=%d id=%d: %v", uid, id, err)
		writeJSON(w, 500, map[string]string{"error": err.Error()})
		return
	}
	log.Printf("[httpapi.DeleteStroke] INFO delete success userID=%d id=%d", uid, id)
...
func (a *API) Recognize(w http.ResponseWriter, r *http.Request) {
	...
	if a.Recognizer == nil {
		writeJSON(w, 503, map[string]string{"error": "recognizer unavailable"})
		return
	}
